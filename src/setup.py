...
def step_optional_integrations() -> None:
    ...
    # Notion (per-user OAuth)
    if _yes_no("Enable Notion (per-user OAuth)?"):
        print("    Each user connects their own Notion workspace via `connect notion` in Slack.")
        print("    You need a Public integration from notion.so/my-integrations.")
        for key, hint in [
